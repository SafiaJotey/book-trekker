...
export default function Review({ book }: { book: IBook }) {
  ...
  for (let i = 0; i < Math.floor(roundedRating); i++) {
    stars.push(<BsStarFill className="text-review text-xl" key={i} />);
  }

  if (roundedRating % 1 !== 0) {
    stars.push(
      <BsStarHalf className="text-review text-xl" key={stars.length} />
    );
  }

  for (let i = stars.length; i < maxRating; i++) {
    stars.push(<BsStar className="text-review text-xl" key={i} />);
  }
